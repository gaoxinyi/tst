				...
				conn.decrby('sku_'+param.goodsId,param.total,(err,v)=>{
					if(err || v<0){
						...
						conn.incrby('seq_order',1,(err,seq_order)=>{
							var coll = new tst.order({_id:seq_date+seq_order,openId:param.openId,yswfCode:param.code,goodsId:param.goodsId,total:param.total,queue:Math.round(Math.random()*4)+1,amount:(vp*param.total*0.925).toFixed(2)*100,original:(vp*param.total).toFixed(2)*100,unitprice:vp*100,addressCode:param.addressCode});
							try{
								coll.save();
